-- offline: платные, один день, для итоговой справки-счета
declare
ctn varchar2(100);
iterator number;
...
if (systemday >= 6) then
 startdate:=2; /*если текущая дата больше 6 числа, инсертим транзакции за текущий месяц (минус 2 дня от текущей даты)*/
else
 startdate:=systemday+15; /*если текущая дата меньше 6 числа, инсертим транзакции за прошлый месяц (середина прошлого месяца)*/
end if;
