-- offline: бесплатные, платные между бесплатными, ACTUAL_SERVICE_PARAMETER3 = NULL
declare
ctn varchar2(100);
iterator number;
...
-- RECHARGE_HISTORY (1 транзакция пополнения во время транзакции мобильного интернета)
core_balance:=core_balance+100;
executeRechargeHistory:='Insert into RECHARGE_HISTORY_'||tablePrefixDwh||' (SUBSCRIBER_ID, RECHARGE_DATE_TIME, CARD_NUMBER, RECHARGE_AMOUNT, EXPIRATION_OFFSET,NEW_BALANCE, VOUCHER_TYPE, BALANCE_TYPE, SERIAL_NUMBER, RECHARGE_COMMENT, CALL_ID, SLICE, BILLSYSTEM_CODE, ISO_CODE, FACE_VALUE, BALANCES_INFO, IDENTITY_ID)
Values ('||ctn||', sysdate-2+'||(iterator+1)||'/(24*60), ''non vouche'', 65, 0, 0, 0, 0, 12070368844, ''XPA,6039818932,1008,2,1111'', 279918750002, 6169982, 23, ''RUR'', 10, ''[1~'||core_balance||'~100~0]'', 1)';
execute immediate executeRechargeHistory;
-- OSA_HISTORY, MERCHANT_ID = GPRS (1 транзакция мобильного интернета, во время которой будет происходить пополнение)
executeOsaHistory:='Insert into OSA_HISTORY_'||tablePrefixDwh||'
   (SUBSCRIBER_ID, START_CALL_DATE_TIME, END_CALL_DATE_TIME, ACTIVITY_TYPE, USAGE_AMOUNT,
   REASON_CODE, ACCOUNT_TYPE, APPLICATION_ID, SUBTYPE_ID, SLU_ID,
   UNIT_TYPE_ID, MERCHANT_ID, SESSION_DESC, CORRELATION_ID, CORRELATION_TYPE,
