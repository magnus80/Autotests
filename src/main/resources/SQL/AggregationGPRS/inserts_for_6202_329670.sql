...
loop
-- OSA_HISTORY, MERCHANT_ID = GPRS, 2 транзакции в первый день
...
executeOsaHistory:='Insert into OSA_HISTORY_'||tablePrefixDwh||'
 ...
 Values
   ('||ctn||', sysdate-3+'||iterator||'/(24*60), sysdate-3+'||iterator||'/(24*60)+1/1440, ''unitBasedCharge'', 1000,
   400, 0, 225, 1541, ''49'',
...
loop
-- OSA_HISTORY, MERCHANT_ID = GPRS, 1 переходящая транзакция (начало в одних сутках, конец в следующих сутках)
core_balance:=core_balance-10;
executeOsaHistory:='Insert into OSA_HISTORY_'||tablePrefixDwh||'
   (SUBSCRIBER_ID, START_CALL_DATE_TIME, END_CALL_DATE_TIME, ACTIVITY_TYPE, USAGE_AMOUNT,
   REASON_CODE, ACCOUNT_TYPE, APPLICATION_ID, SUBTYPE_ID, SLU_ID,
   ...
   PCL_FLAG, ACTUAL_SERVICE_PARAMETER3)
 Values
   ('||ctn||', (trunc(sysdate)-2)-1/(86400/30), (trunc(sysdate)-2)+1/(86400/30), ''unitBasedCharge'', 1000,
   400, 0, 225, 1541, ''49'',
   21, ''GPRS'', ''INT GPRS charging'', 12345, 2,
   1, ''SGSN_IP_ADDRESS:85.115.244.147'', ''''''Internet'''''', ''''''VOLUME_K'''''', 66,
   -1, -1, -1, -1, 2,
   ...
   0, -1)';
iterator:=iterator+1;
exit when iterator>3;
execute immediate executeOsaHistory;
end loop;

-- возвращаем переменные в значени¤ на шаг назад, т.к. они изменились, но был выход из цикла
core_balance:=core_balance+10;
iterator:=iterator-1;

loop
-- OSA_HISTORY, MERCHANT_ID = GPRS, 2 транзакции за второй день
core_balance:=core_balance-10;
executeOsaHistory:='Insert into OSA_HISTORY_'||tablePrefixDwh||'
   (SUBSCRIBER_ID, START_CALL_DATE_TIME, END_CALL_DATE_TIME, ACTIVITY_TYPE, USAGE_AMOUNT,
   REASON_CODE, ACCOUNT_TYPE, APPLICATION_ID, SUBTYPE_ID, SLU_ID,
