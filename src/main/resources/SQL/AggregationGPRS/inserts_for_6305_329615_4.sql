-- offline: платные, один день, для справки-счета
declare
ctn varchar2(100);
iterator number;
...
startdate number; /*значение, которое вычитаем из текущей даты/времени*/
systemday number; /*значение текущего дня, одним числом*/
...
select EXTRACT(day FROM sysdate) into systemday FROM dual;
if (systemday >= 5) then
 startdate:=systemday+15; /*если текущая дата больше 5 числа, инсертим транзакции за прошлый месяц (середина прошлого месяца)*/
else
 startdate:=systemday+45; /*если текущая дата меньше 5 числа, инсертим транзакции за позапрошлый месяц (середина позапрошлого месяца)*/
end if;
...
executeOsaHistory:='Insert into OSA_HISTORY_'||tablePrefixDwh||'
 ...
 Values
   ('||ctn||', sysdate-'||startdate||'+'||iterator||'/(24*60), sysdate-'||startdate||'+'||(iterator+1)||'/(24*60), ''unitBasedCharge'', 1000,
   400, 0, 225, 1541, ''49'',
   21, ''GPRS'', ''INT GPRS charging'', 12345, 2,
   1, ''SGSN_IP_ADDRESS:85.115.244.147'', ''''''Internet'''''', ''''''VOLUME_K'''''', 66,
   -1, -1, -1, -1, 2,
   ''IP85.115.241.7'', 2, ''250996671940230'', ''''''000000;000000;10000#ggsn7.huawei.com;360115520'''''', sysdate-'||startdate||'+'||iterator||'/(24*60),
